...
static gboolean play_next_in_queue(AppData *app) {
    ...
    // Find the corresponding button and set its color
    GList *l;
    for (l = gtk_container_get_children(GTK_CONTAINER(app->playlist_box)); l != NULL; l = l->next) {
        const char *button_label = gtk_button_get_label(GTK_BUTTON(GTK_WIDGET(l->data)));
        if (strcmp(button_label, next_file) == 0) {
            app->current_playlist_item = GTK_WIDGET(l->data);
            GtkStyleContext *context = gtk_widget_get_style_context(app->current_playlist_item);
...
/*oid load_next_video(mpv_handle *mpv, AppData *app) {
...
void load_next_video(mpv_handle *mpv, AppData *app) {
    if (video_queue && video_queue->next) {
        ...
        for (l = video_queue; l != NULL; l = l->next) {
            if (l == current_video) {
                GtkWidget *button = gtk_container_get_children(GTK_CONTAINER(app->playlist_box))->data;
                //find the button with the same label
                GList *children = gtk_container_get_children(GTK_CONTAINER(app->playlist_box));
                for (GList *l2 = children; l2 != NULL; l2 = l2->next) {
                    const char *button_label = gtk_button_get_label(GTK_BUTTON(GTK_WIDGET(l2->data)));
                    if (strcmp(button_label, (char *)l->data) == 0) {
                        button = GTK_WIDGET(l2->data);
                        break;
                    }
                }
                app->current_playlist_item = button;
...
gtk_box_pack_start(GTK_BOX(vbox), hboxa, TRUE, TRUE, 0);
    ...
    GtkWidget *file_icon = gtk_image_new_from_icon_name("document-open", GTK_ICON_SIZE_BUTTON);
    ...
    GtkWidget *subtitle_icon = gtk_image_new_from_icon_name("file-catalog", GTK_ICON_SIZE_BUTTON);
...
if (!subtitle_button) {
    ...
    GtkWidget *fullscreen_icon = gtk_image_new_from_icon_name("gtk-fullscreen", GTK_ICON_SIZE_BUTTON);
